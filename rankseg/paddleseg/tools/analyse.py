...
from rankseg.paddleseg.analyse import analyse
...
def parse_args():
    ...
    # RankSEG optimization params
    parser.add_argument(
        '--use_rankseg',
        action='store_true',
        default=False,
        help='Whether to use RankSEG for optimized segmentation prediction. Default: False.')
    parser.add_argument(
        '--rankseg_metric',
        type=str,
        default='dice',
        choices=['dice', 'iou', 'acc'],
        help='Metric to optimize when using RankSEG. Default: dice.')
    parser.add_argument(
        '--rankseg_solver',
        type=str,
        default='RMA',
        choices=['RMA', 'BA', 'TRNA', 'BA+TRNA'],
        help='Solver for RankSEG optimization. Default: RMA.')
    parser.add_argument(
        '--rankseg_output_mode',
        type=str,
        default='multiclass',
        choices=['multiclass', 'multilabel'],
        help='Output mode for RankSEG. Default: multiclass.')
...
def merge_test_config(cfg, args):
    ...
    # RankSEG parameters
    if args.use_rankseg:
        test_config['use_rankseg'] = args.use_rankseg
        test_config['rankseg_metric'] = args.rankseg_metric
        test_config['rankseg_solver'] = args.rankseg_solver
        test_config['rankseg_output_mode'] = args.rankseg_output_mode
    return test_config
