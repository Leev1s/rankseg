...
def rankdice_ba(
    ...
    for k in range(num_class):
        ...
        for b_idx, b in enumerate(trna_indices):
            ...
            if smooth > 0:
                best_score = smooth * torch.sum((1.0 / (discount[lq[b_idx] : uq[b_idx]] + smooth)) * pmf_tmp)
            ...
            for tau in range(1, up_tau[b, k] + 1):
                ...
                ma_tmp = torch.sum(2.0 / (discount[lq[b_idx] : uq[b_idx]] + tau + smooth + 2) * w_vec)
                nu_tmp = smooth * torch.sum((1.0 / (discount[lq[b_idx] : uq[b_idx]] + tau + smooth + 1)) * pmf_tmp)
...
def rankseg_rma(
    ...
    def convert_to_nonoverlap(
        ...
        increment_scores = torch.where(
            active_mask.unsqueeze(2), increment_scores, torch.full_like(increment_scores, float("-inf"))
        )
